...
class ImageServerDelegate(
	...
	suspend fun isAvailable() = withContext(Dispatchers.Default) {
		getParserRepository()?.let { repository ->
	...
	private fun getParserRepository(): ParserMangaRepository? {
		return mangaSource?.let { mangaRepositoryFactory.create(it) as? ParserMangaRepository }
	}
}
