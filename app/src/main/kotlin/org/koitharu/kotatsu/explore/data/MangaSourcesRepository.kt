...
import org.koitharu.kotatsu.miku.MikuMangaSource
...
class MangaSourcesRepository @Inject constructor(
	...
	val allMangaSources: Set<MangaSource> = Collections.unmodifiableSet(MikuMangaSource.all)

	suspend fun getEnabledSources(): List<MangaSource> {
		assimilateNewSources()
		val order = settings.sourcesSortOrder
		return dao.findAll(!settings.isAllSourcesEnabled, order)
			.toSources(settings.isNsfwContentDisabled, order)
			.map { it.mangaSource }
	}
	...
	private fun observeExternalSources(): Flow<List<MangaSource>> = flowOf(emptyList())

	fun getExternalSources(): List<MangaSource> = emptyList()
	...
	private fun String.toMangaSourceOrNull(): MangaSource? = MikuMangaSource.fromName(this)
}
